...
namespace Eps.CsToDb.Builder.DbModel.Utils
{
    public class DbNamingRules
    {
        ...
        public string MakeColumnName(DbColumnAttribute dbColumnAttribute, Type dbModelType, FieldInfo fieldInfo)
        {
            string columnName;
            ...
            //
            if (DbModelHelpers.IsItPrimaryKeyProperty(_context, dbModelType, fieldInfo) && columnName.Equals("KEY", StringComparison.InvariantCultureIgnoreCase))
            {
        ...
        public static bool ShouldReferencingColumnNameBeChanged(BuilderContext context, DbTable dbTable, DbColumn dbColumn, out string newColumnName)
        {
            ...
            {
                DbTable dbReferencedTable = DbModelHelpers.GetForeignKeyFieldReferencedTable(context, dbTable.SourceType, dbColumn.SourceField);
                ...
                throw new Exception($"Referenced table '{dbReferencedTable.Name}' doesn't have primary key with one column, it cannot be referenced with a property with KEYREF-name from '{dbTable.Name}'.");
            }
            return false;
        }
    }
